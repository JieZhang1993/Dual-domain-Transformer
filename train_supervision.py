...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, help="Path to the config.",
        default='./config/potsdam/dual_domain_transformer.py')
    # arg("-c", "--config_path", type=Path, help="Path to the config.",
    #     default='./config/zeebrugge/iformer_aff_sfam_dl.py')
...
class Supervision_Train(pl.LightningModule):
    ...
    def forward_fm(self, x):
        # only net is used in the prediction/inference
        seg_pre = self.net(x, True)
        return seg_pre

    def forward_lhf_fm(self, x, lhf_fm=0):
        # only net is used in the prediction/inference
        seg_pre = self.net(x, lhf_fm=lhf_fm)
        return seg_pre

    def training_step(self, batch, batch_idx):
        img, mask = batch['img'], batch['gt_semantic_seg']
        # am, im = torch.max(mask), torch.min(mask)
        # print(batch['mask_name'], am, im, end='')
        ...
        # ap, ip = torch.max(pre_mask), torch.min(pre_mask)
        # print(ap, ip)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # TODO: iupdate dl list prediction
        if len(prediction) == 2 and isinstance(prediction, (list, tuple)):
            prediction = prediction[0]
        pre_mask = nn.Softmax(dim=1)(prediction)
        pre_mask = pre_mask.argmax(dim=1)
        for i in range(mask.shape[0]):
            self.metrics_val.add_batch(mask[i].cpu().numpy(), pre_mask[i].cpu().numpy())
            # mi, ma = np.min(mask[i].cpu().numpy()), np.max(mask[i].cpu().numpy())
            # print(mi, ma)
