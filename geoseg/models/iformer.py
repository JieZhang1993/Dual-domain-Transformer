...
from geoseg.models.aff import AFFBlock
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, kernel_size=16, stride=16, padding=0, in_chans=3,
                 embed_dim=768):
        super().__init__()
...
class FirstPatchEmbed(nn.Module):
    ...
    def __init__(self, kernel_size=3, stride=2, padding=1, in_chans=3, embed_dim=768):
        ...
        self.proj1 = nn.Conv2d(in_chans, embed_dim // 2, kernel_size=kernel_size, stride=stride,
                               padding=padding)
        self.norm1 = nn.BatchNorm2d(embed_dim // 2)
        self.gelu1 = nn.GELU()
        self.proj2 = nn.Conv2d(embed_dim // 2, embed_dim, kernel_size=kernel_size, stride=stride,
                               padding=padding)
...
class BaseConv(nn.Module):
    def __init__(self, c1=128, c2=128):
        super().__init__()
...
class HighMixer(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1, padding=1,
        ...
        self.proj1 = nn.Conv2d(cnn_dim, cnn_dim, kernel_size=kernel_size, stride=stride,
                               padding=padding, bias=False, groups=cnn_dim)
...
class HighMixer2(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1, padding=1,
                 **kwargs, ):
        super().__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size, stride=stride, padding=padding)
        self.proj2 = nn.Conv2d(dim, dim * 2, kernel_size=1, stride=1, padding=0)
        self.mid_gelu2 = nn.GELU()
...
class LowMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, C = xa.shape
        qkv = self.qkv(xa).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1,
                                                                                         4)
...
class Mixer(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 attention_head=1, pool_size=2, w_o_dw=False, low_pass=False,
                 **kwargs, ):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim = dim // num_heads
        ...
        self.high_mixer = HighMixer(high_dim) if not w_o_dw else HighMixer2(high_dim)
        self.low_mixer = LowMixer(low_dim, num_heads=attention_head, qkv_bias=qkv_bias,
...
class MixerAff(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 attention_head=1, pool_size=2, w_o_dw=False, only_aff=False,
                 **kwargs, ):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim = dim // num_heads
        ...
        self.conv_fuse = nn.Conv2d(low_dim + high_dim * 2, low_dim + high_dim * 2, kernel_size=3,
                                   stride=1, padding=1, bias=False, groups=low_dim + high_dim * 2)
        self.proj = nn.Conv2d(low_dim + high_dim * 2 + dim, dim, kernel_size=1, stride=1, padding=0)
        self.proj_drop = nn.Dropout(proj_drop)

        self.aff = AFFBlock(dim, dim, int(dim * 2))
    ...
    def forward(self, x, fm=False):
        ...
        x = self.proj(torch.cat([x, x_aff], 1))
        fms.append(x)
        x = self.proj_drop(x)

        x = x.permute(0, 2, 3, 1).contiguous()

        return x if not fm else fms


class MixerOnlyAff(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 attention_head=1, pool_size=2, w_o_dw=False, **kwargs, ):
        super().__init__()
        self.aff = AFFBlock(dim, dim, int(dim * 2))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 pool_size=2,
                 attn=Mixer, w_o_dw=False, low_pass=False,
                 use_layer_scale=False, layer_scale_init_value=1e-5, only_hp=False, only_aff=False,
                 ):
        super().__init__()
        ...
        if only_hp:
            self.attn = MixerOnlyHigh(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                      attention_head=attention_head, pool_size=pool_size, w_o_dw=w_o_dw,
                                      low_pass=low_pass)
        else:
            self.attn = attn(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                             attention_head=attention_head, pool_size=pool_size, w_o_dw=w_o_dw, low_pass=low_pass)
...
class BlockAff(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_head=1,
                 pool_size=2,
                 attn=MixerAff, w_o_dw=False, low_pass=False,
                 use_layer_scale=False, layer_scale_init_value=1e-5, only_hp=False, only_aff=False,
                 ):
        super().__init__()
        ...
        self.norm1 = norm_layer(dim)
        if only_aff:
            self.attn = MixerOnlyAff(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                     attention_head=attention_head, pool_size=pool_size, w_o_dw=w_o_dw)
        else:
            self.attn = attn(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                             attention_head=attention_head, pool_size=pool_size, w_o_dw=w_o_dw)
    ...
    def forward(self, x, fm=False):
        if fm:
            return self.attn(self.norm1(x), fm)
        if self.use_layer_scale:
            x = x + self.drop_path(self.layer_scale_1 * self.attn(self.norm1(x)))
            x = x + self.drop_path(self.layer_scale_2 * self.mlp(self.norm2(x)))
        else:
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None,
                 ...
                 checkpoint_path=None, w_o_dw=False, aff=False, low_pass=False, only_hp=False, only_aff=False,
                 **kwargs,
                 ):
        ...
        self.channel_align = nn.ModuleList(
            [BaseConv(i, o) for i, o in zip(embed_dims, [64, 128, 256, 512])])
        ...
        block_ = Block if not aff else BlockAff
        ...
        self.blocks1 = nn.Sequential(*[
            Block(
                dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                act_layer=act_layer, attention_head=attention_heads[i], pool_size=2, w_o_dw=w_o_dw, low_pass=low_pass,
                only_hp=only_hp,
                only_aff=only_aff
            )
        ...
        self.blocks2 = nn.Sequential(*[
            block_(
                dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                act_layer=act_layer, attention_head=attention_heads[i], pool_size=2, w_o_dw=w_o_dw, low_pass=low_pass,
                only_hp=only_hp,
        ...
        self.blocks3 = nn.Sequential(*[
            block_(
                dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                act_layer=act_layer, attention_head=attention_heads[i], pool_size=1,
                use_layer_scale=use_layer_scale, layer_scale_init_value=layer_scale_init_value,
                w_o_dw=w_o_dw, low_pass=low_pass, only_hp=only_hp, only_aff=only_aff
            )
        ...
        self.blocks4 = nn.Sequential(*[
            block_(
                dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                act_layer=act_layer, attention_head=attention_heads[i], pool_size=1,
                use_layer_scale=use_layer_scale, layer_scale_init_value=layer_scale_init_value,
    ...
    def forward_features(self, x, fmi=0):
        # fmi = 2 or 3 or 4, 才起作用，
        out = []
        fm_out = None
        ...
        out.append(self.channel_align[0](x))
        ...
        x = self.blocks2(x)
        if fmi == 2:
            fm_out = self.blocks2[-1](x, fm=True)  # TODO
        x = x.permute(0, 3, 1, 2).contiguous()
        out.append(self.channel_align[1](x))
        ...
        x = self.blocks3(x)
        if fmi == 3:
            fm_out = self.blocks3[-1](x, fm=True)  # TODO
        x = x.permute(0, 3, 1, 2).contiguous()
        out.append(self.channel_align[2](x))
        ...
        x = self.blocks4(x)
        if fmi == 4:
            fm_out = self.blocks4[-1](x, fm=True)  # TODO
        x = x.permute(0, 3, 1, 2).contiguous()
        out.append(self.channel_align[3](x))

        return tuple(out) if fmi not in [2, 3, 4] else fm_out

    def forward(self, x, fmi=0):
        x = self.forward_features(x, fmi)
        return x
...
def iformer_small(pretrained=False, img_size=512, **kwargs):
    """
    ...
    model = InceptionTransformer(img_size=img_size,
                                 ...
                                 **kwargs)
    if pretrained:
        checkpoint = torch.load('./iformer_small_384.pth',
                                map_location="cpu")

        # "pos_embed1", "pos_embed2", "pos_embed3", "pos_embed4". pos embed
        for i in range(1, 5, 1):
            x = img_size // 2 ** (i + 1)
            pos_weight = checkpoint[f"pos_embed{i}"].permute(0, 3, 1, 2)
            pos_weight = torch.nn.functional.interpolate(pos_weight, (x, x),
                                                         mode='bilinear').permute(0, 2, 3, 1)
            checkpoint[f"pos_embed{i}"] = pos_weight
        checkpoint = intersect_dicts(checkpoint, model.state_dict())
...
def intersect_dicts(da, db, exclude=()):
    ...
    return {k: v for k, v in da.items() if
            k in db and not any(x in k for x in exclude) and v.shape == db[k].shape}
...
if __name__ == '__main__':
    model = iformer_small(pretrained=False, w_o_dw=True)
