...
class CSWinT(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """

    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64, depth=[1, 2, 21, 1],
                 split_size=[1, 2, 7, 7], num_heads=[2, 4, 8, 16], mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        self.merge1 = Merge_Block(curr_dim, curr_dim * (heads[1] // heads[0])) if not fcm else FCM(curr_dim)
        ...
        self.merge2 = Merge_Block(curr_dim, curr_dim * (heads[2] // heads[1])) if not fcm else FCM(curr_dim)
        ...
        self.merge3 = Merge_Block(curr_dim, curr_dim * (heads[3] // heads[2])) if not fcm else FCM(curr_dim)
        ...
        self.init_weights('./cswin_tiny_224.pth')
        ...
        if isinstance(pretrained, str):
            ...
            if checkpoint_path and os.path.isfile(checkpoint_path):
                ...
                if isinstance(checkpoint, dict):
                    if True and 'state_dict_ema' in checkpoint:
...
if __name__ == '__main__':
    ...
    checkpoint_path = './cswin_tiny_224.pth'
    if checkpoint_path and os.path.isfile(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        state_dict_key = 'state_dict'
        model_key = 'model'
