...
def train_aug(img, mask):
    ...
    crop_aug = Compose([RandomScale(scale_list=[0.5, 0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV1(crop_size=512, max_ratio=0.75,  # 0.75
...
class VaihingenDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if p_ratio > self.mosaic_ratio or self.mode == 'val' or self.mode == 'test':
            ...
            if self.transform:
                img, mask = self.transform(img, mask)
        mask_name = osp.join(self.data_root, self.mask_dir, self.img_ids[index] + self.mask_suffix)
        img = torch.from_numpy(img).permute(2, 0, 1).float()
        mask = torch.from_numpy(mask).long()
        img_id = self.img_ids[index]
        results = dict(img_id=img_id, img=img, gt_semantic_seg=mask, mask_name=mask_name)
        return results
