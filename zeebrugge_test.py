...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [0, 0, 129]
...
def img_writer(inp):
    ...
    if rgb:
        ...
        cv2.imwrite(mask_name_tif, mask_tif[..., ::-1])
    else:
...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, default='config/zeebrugge/dual_domain_transformer.py', help="Path to  config")
    arg("-o", "--output_path", type=Path, default='fig_results/zeebrugge/dual_domain_transformer', help="Path where to save resulting masks.")
    arg("-t", "--tta", help="Test time augmentation.", default=None, choices=[None, "d4", "lr"])
    arg("--rgb", help="whether output rgb images", default=True)
...
def main():
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            if len(raw_predictions) == 2 and isinstance(raw_predictions, (list, tuple)):
