...
def parse_args():
    ...
    parser.add_argument("--mask-dir", default="../data/zeebrugge/train_masks")
    parser.add_argument("--output-img-dir", default="../data/zeebrugge/train/images_512")
    parser.add_argument("--output-mask-dir", default="../data/zeebrugge/train/masks_512")
...
def image_augment(image, mask, patch_size, mode='train', val_scale=1.0, mask_path=None):
    image_list = []
    mask_list = []
    ...
    if mode == 'train':
        ...
        mask = np.clip(np.array(mask.copy()) - 1, 0, 7)
        image, mask = get_img_mask_padded(image.copy(), mask, patch_size,
                                          mode)  # 转换labels(1-8)->(0-7)
        mask = mask[..., -1]
...
def patch_format(inp):
    ...
    # print(img)
    npf = np.fromfile(mask_path, np.uint8)
    npf = np.reshape(npf, newshape=(10000, 10000))
    a11, b11 = np.max(npf), np.min(npf)
    mask = Image.fromarray(npf, mode='L').convert('RGB')
    maska11, maskb11 = np.max(mask), np.min(mask)
    ...
    image_list, mask_list = image_augment(image=img.copy(), mask=mask.copy(), patch_size=split_size,
                                          val_scale=val_scale, mode=mode, mask_path=mask_path)
            ...
            for x in range(0, img.shape[1], stride):
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    if cf[4] > 1.0 and mode == 'train':  # ignore car_aug, no improvement
                        ...
                        out_img_path = os.path.join(imgs_output_dir, "{}_{}_{}.tif".format(img_filename[:-4], m, k))
                        cv2.imwrite(out_img_path, img_tile)
                        # mask_filename = mask_filename.replace('raw', 'tif')
                        out_mask_path = os.path.join(masks_output_dir, "{}_{}_{}.png".format(mask_filename[:-4], m, k))
...
if __name__ == "__main__":
    ...
    mask_paths_raw = glob.glob(os.path.join(masks_dir, "*.raw"))
    ...
    after_aug_img = glob.glob(f'{masks_output_dir}/*.*')
    print('after_aug_img numbers:', len(after_aug_img))
