...
# crop 256, training size 256
# split huge RS image to small patches
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--img-dir", default="../data/vaihingen/test_images")
    parser.add_argument("--mask-dir", default="../data/vaihingen/test_masks_eroded")
    parser.add_argument("--output-img-dir", default="../data/vaihingen/test/images_512")
    parser.add_argument("--output-mask-dir", default="../data/vaihingen/test/masks_512")
    parser.add_argument("--eroded", type=bool, default=True)
...
def randomsizedcrop(image, mask):
    ...
    crop = albu.RandomSizedCrop(min_max_height=(int(3 * h // 8), int(h // 2)), width=h, height=w)(image=image.copy(),
                                                                                                  mask=mask.copy())
...
if __name__ == "__main__":
    ...
    # img_paths.sort(key=lambda i: int(i.split('area')[-1].split('.tif')[0]))
    # mask_paths.sort(key=lambda i: int(i.split('area')[-1].split('_noBoun')[0]))
    img_paths.sort()
    mask_paths.sort()
